    ...
    def loader_forward(
        ...
        for batch_idx, (x, y) in enumerate(loader):
            ...
            if (batch_idx % log_freq == 0) and train:
                logger.info(
                    "Batch %s/%s, Loss: %s, LL R1: %s, LL R2: %s, MSE: %s, Res corr: %s, Res corr std: %s",
                    batch_idx,
                    len(loader),
                    loss.item(),
                    r1_ll.item(),
        ...
        if not train:
            ...
            axes[0, 2].hist(r2_res_np, bins=100)
            axes[0, 2].set_title("R2 Residuals (scaled)")
            axes[0, 3].scatter(r1_res_np, r2_res_np)
            axes[0, 3].set_title("R1 Residuals vs R2 Residuals (scaled)")
            axes[1, 0].scatter(y_r1_np, y_r2_np)
            axes[1, 0].set_title("R1 (obs) vs R1 (obs)")
            axes[1, 1].scatter(y_r1_np, r1_dist_np)
            axes[1, 1].set_title("R1 (obs) vs R1 (pred)")
            axes[1, 2].scatter(y_r1_np, x_r1_np[:, 0])
            axes[1, 2].set_title("R1 (obs) vs R1 (ctrl)")
            axes[1, 3].scatter(r1_dist_np, x_r1_np[:, 0])
            ...
            # ])
            # val_table.add_data(
            #     x_r1_np[:, 0], x_r1_np[:, 1], x_r1_np[:, 2], x_r1_np[:, 3], x_r1_np[:, 4],
            #     x_r2_np[:, 0], x_r2_np[:, 1], x_r2_np[:, 2], x_r2_np[:, 3], x_r2_np[:, 4],
            #     y_r2_np, y_r2_np / y_r1_np,
            #     r1_dist_np, r2_dist_np,
            #     r1_std_np, r2_std_np,
            # )
            # wandb.log({
            #     "val_table": val_table,
...
if __name__ == "__main__":
    ...
    run_name = f"{args.target}_mu_{'_'.join(map(str, args.mu_dims))}_r_{'_'.join(map(str, args.r_dims))}_wd_{args.weight_decay}_lr_{args.lr}_warmup_{args.warmup_epochs}"
